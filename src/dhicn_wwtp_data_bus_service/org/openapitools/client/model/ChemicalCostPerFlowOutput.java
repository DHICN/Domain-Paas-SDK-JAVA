/*
 * 污水业务中台领域服务
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 1.0.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */


package org.openapitools.client.model;

import java.util.Objects;
import java.util.Arrays;
import com.google.gson.TypeAdapter;
import com.google.gson.annotations.JsonAdapter;
import com.google.gson.annotations.SerializedName;
import com.google.gson.stream.JsonReader;
import com.google.gson.stream.JsonWriter;
import io.swagger.annotations.ApiModel;
import io.swagger.annotations.ApiModelProperty;
import java.io.IOException;
import java.util.ArrayList;
import java.util.List;
import org.openapitools.client.model.Data;

/**
 * ChemicalCostPerFlowOutput
 */
@javax.annotation.Generated(value = "org.openapitools.codegen.languages.JavaClientCodegen", date = "2023-08-09T14:11:26.479568400+08:00[Asia/Shanghai]")
public class ChemicalCostPerFlowOutput {
  public static final String SERIALIZED_NAME_CHEMICAL_NAME = "chemicalName";
  @SerializedName(SERIALIZED_NAME_CHEMICAL_NAME)
  private String chemicalName;

  public static final String SERIALIZED_NAME_DATAS = "datas";
  @SerializedName(SERIALIZED_NAME_DATAS)
  private List<Data> datas = null;


  public ChemicalCostPerFlowOutput chemicalName(String chemicalName) {
    
    this.chemicalName = chemicalName;
    return this;
  }

   /**
   * 药剂名称 dosage name
   * @return chemicalName
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "药剂名称 dosage name")

  public String getChemicalName() {
    return chemicalName;
  }


  public void setChemicalName(String chemicalName) {
    this.chemicalName = chemicalName;
  }


  public ChemicalCostPerFlowOutput datas(List<Data> datas) {
    
    this.datas = datas;
    return this;
  }

  public ChemicalCostPerFlowOutput addDatasItem(Data datasItem) {
    if (this.datas == null) {
      this.datas = new ArrayList<Data>();
    }
    this.datas.add(datasItem);
    return this;
  }

   /**
   * 每条工艺线的吨水药耗数据 dosage cost per ton water of each product line
   * @return datas
  **/
  @javax.annotation.Nullable
  @ApiModelProperty(value = "每条工艺线的吨水药耗数据 dosage cost per ton water of each product line")

  public List<Data> getDatas() {
    return datas;
  }


  public void setDatas(List<Data> datas) {
    this.datas = datas;
  }


  @Override
  public boolean equals(java.lang.Object o) {
    if (this == o) {
      return true;
    }
    if (o == null || getClass() != o.getClass()) {
      return false;
    }
    ChemicalCostPerFlowOutput chemicalCostPerFlowOutput = (ChemicalCostPerFlowOutput) o;
    return Objects.equals(this.chemicalName, chemicalCostPerFlowOutput.chemicalName) &&
        Objects.equals(this.datas, chemicalCostPerFlowOutput.datas);
  }

  @Override
  public int hashCode() {
    return Objects.hash(chemicalName, datas);
  }


  @Override
  public String toString() {
    StringBuilder sb = new StringBuilder();
    sb.append("class ChemicalCostPerFlowOutput {\n");
    sb.append("    chemicalName: ").append(toIndentedString(chemicalName)).append("\n");
    sb.append("    datas: ").append(toIndentedString(datas)).append("\n");
    sb.append("}");
    return sb.toString();
  }

  /**
   * Convert the given object to string with each line indented by 4 spaces
   * (except the first line).
   */
  private String toIndentedString(java.lang.Object o) {
    if (o == null) {
      return "null";
    }
    return o.toString().replace("\n", "\n    ");
  }

}

